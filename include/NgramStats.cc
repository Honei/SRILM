...
void NgramCounts<CountT>::dump() {
    cerr << "order = " << order << endl;
    counts.dump();
}

template <class CountT>
void NgramCounts<CountT>::memStats(MemStats &stats) {
 ...
 * **/
template <class CountT>
unsigned int NgramCounts<CountT>::parseNgram(char *line, VocabString *words,
		      unsigned int max, CountT &count) {
	// 解析得到单词，返回单词的数目，分割之后的单词保存在 words 中
	...
	// 返回单词的数目，将 words 的最后一个维度置零
    return howmany;
}

/**
 * 从训练文件中读取一行，并且解析出来
 * @param [in] file: 训练文件句柄
 * @param [in] words: 读取的单词
 * @param [in] max: 
 * @param [in] count: 
 * **/
template <class CountT>
unsigned int NgramCounts<CountT>::readNgram(File &file, VocabString *words,
											unsigned int max, CountT &count) {
    char *line;
	...
	// 如果返回了0，那么表示这一行的单词超过了最大单词的限制 max
    if (howmany == 0) {
 ...
 * 读取统计数据
 * @param [in] file: 文件句柄
 * @param [in] order: n-gram 的阶数
 * @param [in] limitVocab: 
 * **/
template <class CountT>
Boolean NgramCounts<CountT>::read(File &file, unsigned int order, Boolean limitVocab) {
	cout << "read the NgramCounts, order: " << order << ", limitVocab: " << limitVocab << endl;
	...
	// 解析一行文件中的内容，返回读取单词的数目 howmany，words 中保存了解析之后的单词
	// count 表示这一行中统计的数据
    while ((howmany = readNgram(file, words, maxNgramOrder + 1, count))) {
		...
		*/
		if (limitVocab) {
			...
			*/
			...
			// 添加新的词
			// cout << "words: " << *words << ", wids: " << *wids << ", maxNgramOrder: " << maxNgramOrder << endl;
		...
		*  Update the count
		*/
		CountT *cnt = intersect ? counts.find(wids) : counts.insert(wids);
	...
	// cout << "word count: " << vocab.numWords() << endl;
	// if (debug(DEBUG_READ_GOOGLE)) {
	//     cout << "word count: " << vocab.numWords() << endl;
	// 	for (int i = 0; i < vocab.numWords(); i++) {
	// 		cout << "the " << i << " word is: " << vocab.getWord(i) << endl; 
	// 	}
	// }
